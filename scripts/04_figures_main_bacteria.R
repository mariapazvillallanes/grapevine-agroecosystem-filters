...
# Purpose:
#   Generate MAIN figures for culturable bacteria (paper-ready):
...
#   - Fig4: PCoA (Jaccard) using objects generated in 02_bacteria_analysis.R
#
# Inputs:
#   data/processed/Bacterias_withSampleID.csv
#   outputs/objects/bacteria_meta.rds
#   outputs/objects/bacteria_pcoa_jaccard.rds
#   outputs/objects/bacteria_dist_jaccard.rds   (optional; not required here)
#
# Outputs:
#   outputs/figures/main/Fig1_BacterialComposition.pdf
#   outputs/figures/main/Fig2_ExclusiveSharedClasses.pdf
#   outputs/figures/main/Fig3_DeltaRichness.pdf
#   outputs/figures/main/Fig4_PCoA_Jaccard.pdf
#   outputs/tables/Table_S1_DeltaRichness_Bacteria.csv
#   outputs/tables/ExclusiveClasses_Detailed_Bacteria.csv
# ======================================================================
...
# ---- Paths ----
infile <- "data/processed/Bacterias_withSampleID.csv"

dir.create("outputs/figures/main", recursive = TRUE, showWarnings = FALSE)
dir.create("outputs/tables",       recursive = TRUE, showWarnings = FALSE)

# ---- Consistent factor levels (as in your manuscript) ----
...
df <- readr::read_delim(infile, delim = ";", show_col_types = FALSE, trim_ws = TRUE) %>%
  mutate(
    ...
    BacterialClass = str_trim(BacterialClass),
...
# 1) Isolates per plant x class
plant_class <- df %>%
  group_by(PlantID, Management, Organ, Medium, BacterialClass) %>%
  summarise(Isolates = sum(Count, na.rm = TRUE), .groups = "drop")

# Totals per plant (for composition) + recovery flag
...
all_classes <- sort(unique(plant_comp$BacterialClass))

# Complete absent classes with 0 (only for recovered plants)
plant_comp0 <- plant_totals %>%
  filter(Present) %>%
  select(PlantID, Management, Organ, Medium, Total, Present) %>%
  left_join(
    plant_comp %>% select(PlantID, Management, Organ, Medium, BacterialClass, Freq),
    by = c("PlantID", "Management", "Organ", "Medium")
  ) %>%
  group_by(PlantID, Management, Organ, Medium, Total, Present) %>%
  tidyr::complete(BacterialClass = all_classes, fill = list(Freq = 0)) %>%
  ungroup()

# Choose Top N classes based on global mean (including zeros)
top_n <- 6
top_classes <- plant_comp0 %>%
  group_by(BacterialClass) %>%
  summarise(GlobalMean = mean(Freq), .groups = "drop") %>%
  arrange(desc(GlobalMean)) %>%
  slice_head(n = top_n) %>%
  pull(BacterialClass)

# Collapse others to "Other" per plant (sum to 1)
plant_comp2 <- plant_comp0 %>%
  mutate(Class_plot = ifelse(BacterialClass %in% top_classes, BacterialClass, "Other")) %>%
...
# Mean composition per condition
...
# Panel labels (n recovered / total plants)
...
# Class palette (auto-expand if top_n changes)
...
p_fig1 <- ggplot(plot_df1, aes(x = Organ, y = MeanFreq * 100, fill = Class_plot)) +
  ...
  labs(x = NULL, y = "Mean composition (%)", fill = "Bacterial class") +
  geom_text(
    data = panel_counts,
...
ggsave("outputs/figures/main/Fig1_BacterialComposition.pdf",
...
pa <- df %>%
  group_by(Organ, Management, Medium, PlantID, BacterialClass) %>%
  summarise(present = sum(Count, na.rm = TRUE) > 0, .groups = "drop") %>%
  group_by(Organ, Management, Medium, BacterialClass) %>%
  summarise(present = any(present), .groups = "drop")

wide <- pa %>%
  mutate(group = paste(Organ, Management, sep = " · ")) %>%
  select(group, Organ, Management, BacterialClass, Medium, present) %>%
  tidyr::complete(group, Organ, Management, BacterialClass, Medium, fill = list(present = FALSE)) %>%
...
ggsave("outputs/figures/main/Fig2_ExclusiveSharedClasses.pdf",
       p_fig2, width = 18, height = 12, units = "cm", device = cairo_pdf)

# Export detailed list (useful for supplement / transparency)
exclusive_classes_detailed <- wide %>%
  ...
  filter(!is.na(Category)) %>%
  select(Organ, Management, Category, BacterialClass) %>%
  arrange(Management, Organ, Category, BacterialClass)

write.csv2(exclusive_classes_detailed, "outputs/tables/ExclusiveClasses_Detailed_Bacteria.csv", row.names = FALSE)
...
rich_long <- df %>%
  group_by(PlantID, Management, Organ, Medium) %>%
  summarise(
    Richness = n_distinct(BacterialClass[Count > 0]),
    .groups = "drop"
  )
...
write.table(stats_df,
            file = "outputs/tables/Table_S1_DeltaRichness_Bacteria.csv",
...
ggsave("outputs/figures/main/Fig3_DeltaRichness_Bacteria.pdf",
...
meta_bac <- readRDS("outputs/objects/bacteria_meta.rds")
pcoa_bac <- readRDS("outputs/objects/bacteria_pcoa_jaccard.rds")

# In analysis we excluded N=0 before building dist/jaccard PCoA
meta_ord <- meta_bac %>% filter(N > 0)

# Extract % variance (positive eigenvalues only)
eig <- pcoa_bac$eig
var1 <- 100 * eig[1] / sum(eig[eig > 0])
var2 <- 100 * eig[2] / sum(eig[eig > 0])

ord <- as.data.frame(pcoa_bac$points)
...
ggsave("outputs/figures/main/Fig4_PCoA_Jaccard_Bacteria.pdf",
       p_fig4, width = 7.2, height = 5.2, device = cairo_pdf)

message("Main bacteria figures exported to outputs/figures/main/")
